...
const app = {
  ...
  renderMenu: function (menuItems) {
    ...
    document.querySelector(".section-center").innerHTML = html.join("");
  },
  renderTab: function () {
    const tabs = this.menu.reduce(
      (values, item) => {
    ...
    const tabBtn = tabs
      .map((tab) => {
        return `
      <button class="filter-btn" type="button" data-id=${tab}>
      ${tab}
      </button>`;
      })
      .join("");
    listBtn.innerHTML = tabBtn;
  },
  handleEvents: function () {
    ...
    categories.forEach((category) => {
      category.onclick = function (e) {
        ...
        const menuFilter = _this.menu.filter((item) => {
          if (item.category === categoryId) {
            return item;
          }
        });
        if (categoryId === "all") {
          _this.renderMenu(_this.menu);
        } else {
          _this.renderMenu(menuFilter);
        }
      };
    });
  },
